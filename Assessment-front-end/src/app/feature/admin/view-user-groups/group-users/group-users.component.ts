...
export class GroupUsersComponent {
  ...
  constructor(
    ...
    private dialogref: MatDialogRef<GroupUsersComponent>,
  ...
  ngOnInit() {
    ...
    if (this.data?.selectedUsers) {
      this.selectedUsers = JSON.parse(this.data?.selectedUsers);
      if (this.data?.users) {
      ...
      this.selectedUsers = JSON.parse(this.data?.userList);
      if (this.data?.users) {
        this.users = JSON.parse(this.data?.users);
        this.selectedUsers.forEach(selectedUser => {
          const index = this.users.findIndex(user => user.id === selectedUser.id);
