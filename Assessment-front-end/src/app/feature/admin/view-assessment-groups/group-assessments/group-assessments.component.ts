...
export class GroupAssessmentsComponent {
  ...
  constructor(
    ...
    private dialogref: MatDialogRef<GroupAssessmentsComponent>,
  ...
  ngOnInit() {
    ...
    this.assessmentData = this.data?.assessments
    if (this.data?.selectedAssessments) {
      this.selectedAssessments = JSON.parse(this.data?.selectedAssessments);
      if (this.data?.assessments) {
        this.assessments = JSON.parse(this.data?.assessments);
        // Remove selected assessments from the assessments array
        this.selectedAssessments.forEach(selectedAssessment => {
          const index = this.assessments.findIndex(assessment => assessment.id === selectedAssessment.id);
          if (index !== -1) {
      ...
      this.groupName = JSON.parse(this.data?.groupName);
      this.selectedAssessments = JSON.parse(this.data?.assessmentList);
      if (this.data?.assessments) {
        this.assessments = JSON.parse(this.data?.assessments);
        this.selectedAssessments.forEach(selectedAssessment => {
          const index = this.assessments.findIndex(assessment => assessment.assessmentId === selectedAssessment.assessmentId);
      ...
      if (this.data.assessments) {
        this.assessments = JSON.parse(this.assessmentData);
        this.selectedAssessments.forEach(selectedAssessment => {
          const index = this.assessments.findIndex(assessment => assessment.assessmentId === selectedAssessment.assessmentId);
          if (index !== -1) {
            this.assessments.splice(index, 1);
