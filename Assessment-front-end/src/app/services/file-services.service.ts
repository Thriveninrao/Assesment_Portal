...
export class FileServicesService {
  ...
  downloadXLSXFile(assessmentId: number): Observable<Blob> {
    return this.http.get(
      `http://localhost:9901/assessment/ExportAllQuestions/${assessmentId}`,
      { responseType: 'blob' }
    );
  }
  ...
  setMarks(marksObtained: number,assignId:number): Observable<any> {
    const url = `http://localhost:9901/setMarks/${marksObtained}/${assignId}`;
